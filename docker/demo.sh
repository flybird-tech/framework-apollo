...
if [ -z ${config-db-host} ];then
    config-db-host=mysql
fi
if [ -z ${config-db-port} ];then
    config-db-port=3306
fi
if [ -z ${portal-db-host} ];then
    portal-db-host=mysql
fi
if [ -z ${portal-db-port} ];then
    portal-db-port=3306
fi

# apollo config db info
apollo_config_db_url=jdbc:mysql://${config-db-host}:${config-db-port}/ApolloConfigDB?characterEncoding=utf8
apollo_config_db_username=ApolloConfigDB
apollo_config_db_password=123456

# apollo portal db info
apollo_portal_db_url=jdbc:mysql://${portal-db-host}:${portal-db-port}/ApolloPortalDB?characterEncoding=utf8
